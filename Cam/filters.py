...
class VStrokeEdges(object):
    def __init__(self, blurKsize=7, edgeKsize=5):
        self.__blurKsize = blurKsize
        self.__edgeKsize = edgeKsize

    def apply(self, src, dst):
        if self.__blurKsize >= 3:
            blurredSrc = cv2.medianBlur(src, self.__blurKsize)
            graySrc = cv2.cvtColor(blurredSrc, cv2.COLOR_BGR2GRAY)
        else:
            graySrc = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
        cv2.Laplacian(graySrc, cv2.CV_8U, graySrc, ksize=self.__edgeKsize)
